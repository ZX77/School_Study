...
class FightFrame extends Frame implements Runnable, KeyListener {
    Toolkit toolkit = Toolkit.getDefaultToolkit();
    int djx = new Random().nextInt(300), djy = 10; //敌机的X坐标,Y坐标
    int mex = 90, mey = 440; // 我方飞机的X坐标,Y坐标
    int bx = mex + 22, by = mey; //子弹的X坐标,Y坐标

    Image imgdj = toolkit.getImage("images/dj.gif"); //得到敌机图片对象
    Image imgme = toolkit.getImage("images/main.gif"); //得到我方飞机的图片对象
    ...
    public void paint(Graphics g) {
        if (gameover) { //是否结束游戏,如果为true表示结束,调飞机爆炸的图片
            g.drawImage(toolkit.getImage("images/dj_over.gif"), djx, djy, this);
            g.drawImage(toolkit.getImage("images/me_over.gif"), mex, 450, this);
