	...
	class ActionHandler implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			if(e.getSource()==btnReg){
				...
				account.setPwd(txtPwd.getText().trim());
				account.setNickName(txtNickName.getText().trim());
				account.setBlood(sBlood[rbBlood.getSelectedIndex()]);
				//兴趣爱好:玩游戏,上网,吃货,旅游,打篮球
				String str ="";
				if(cbHobit1.isSelected()){
					str = "玩游戏,";
				}
				if(cbHobit2.isSelected()){
					str += "上网,";
				}
				if(cbHobit3.isSelected()){
					str += "吃货,";
				}
				if(cbHobit4.isSelected()){
					str += "旅游,";
				}
				if(cbHobit5.isSelected()){
					str += "打篮球";
				}
				account.setHobit(str);
				account.setJob(txtJob.getText().trim());
				account.setNation(sNation[cbNation.getSelectedIndex()]);
				account.setRemark(taSign.getText().trim());
				account.setStar(sStar[rbStar.getSelectedIndex()]);
				
				//与底层的DAO建立联系，把数据保存到数据库
				
				AccountADO accDao = new AccountDaoImpl();
				accDao.add(account);
				//测试查询结果
				new FindUI();
